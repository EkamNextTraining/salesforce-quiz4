...
function isQuizActive() {
  ...
  return screen && screen.id === "screen-quiz";
}
...
// 1. Detect tab switch / window minimise
document.addEventListener("visibilitychange", () => {
  if (document.hidden && isQuizActive()) {
    tabSwitchCount++;
    if (tabSwitchCount >= MAX_TAB_SWITCHES) {
      alert("⚠️ Assessment auto-submitted: too many tab switches detected.");
      doSubmit();
    } else {
      sessionStorage.setItem(
        "sfquiz_tabwarn",
        `⚠️ Warning ${tabSwitchCount}/${MAX_TAB_SWITCHES}: Switching tabs is not allowed. Your assessment will be auto-submitted after ${MAX_TAB_SWITCHES} violations.`
      );
    }
  }
  if (!document.hidden) {
    const warn = sessionStorage.getItem("sfquiz_tabwarn");
    if (warn) { alert(warn); sessionStorage.removeItem("sfquiz_tabwarn"); }
  }
});

// 2. Warn before page close / refresh / back button
...
// 3. Disable right-click during quiz
document.addEventListener("contextmenu", (e) => {
  if (isQuizActive()) {
    e.preventDefault();
    showAntiCheatWarning("Right-click is disabled during the assessment.");
  }
});

// 4. Block keyboard shortcuts: new tab, new window, close tab, reload, view source, DevTools
document.addEventListener("keydown", (e) => {
  ...
  if (
    ...
    e.preventDefault();
    showAntiCheatWarning("Keyboard shortcut blocked during assessment.");
  }
});
